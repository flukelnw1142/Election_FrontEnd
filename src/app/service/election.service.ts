...
export class ElectionService {
  ...
  getResults(): Observable<{ [province: string]: string }> {
    return this.http
      .get<{ [province: string]: { [constituency: string]: any } }>(
        '/assets/election-results.json'
      )
      .pipe(
        map((data) => {
          ...
          for (const province in data) {
            ...
            for (const constituency in data[province]) {
              const party =
                data[province][constituency].DISTRICT.rank1.party_name ||
                'Unknown';
              partyCounts[party] = (partyCounts[party] || 0) + 1;
            }
            const winner = Object.keys(partyCounts).reduce(
              (a, b) => (partyCounts[a] > partyCounts[b] ? a : b),
              'Unknown'
            );
  ...
  // ดูลำดับแรกของแต่ละเขต
  getConstituencies(
    province: string
  ): Observable<{ [constituency: string]: string }> {
    return this.http
      .get<{ [province: string]: { [constituency: string]: any } }>(
        '/assets/election-results.json'
      )
